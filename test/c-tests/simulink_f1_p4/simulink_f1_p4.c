...
/*****************************/
// property 4
// "Lights should never remain lit infinitely."
/*****************************/

void p4(_Bool inputs[UNFOLDINGS][6]) {
  int d = UNFOLDINGS;
  ind=-1;
  cpt=0;
  int countL = 0; // nb of consecutive cycles with outL == TRUE
  int countR = 0; // nb of consecutive cycles with outR == TRUE
  ...
  // lights do not remain always lit
  assert(countL < d && countR < d);
}
...
int main() {
  ...
  inputs[5][3] = 1; // fermeture des portes (ouvertes)

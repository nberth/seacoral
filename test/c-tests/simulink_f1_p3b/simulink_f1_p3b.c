...
/*****************************/
// property 3b
// "When the F signal is off, then the Flashers_left, Flashers_right and Warning
// functions are disabled."
/*****************************/

void p3b(_Bool inputs[UNFOLDINGS][6]) {
  int d = UNFOLDINGS;
  ind=-1;
  cpt=0;
  ...
  for(int i = 0; i < UNFOLDINGS; i++) {
    for(int j = 0; j < 6; j++) {
      tabin[i][j] = inputs[i][j];
    }
  }
  ...
  while (cpt < d) {
    ...
    W=nondet_in();
    // preconditions
    // LK = FALSE; // Lock function inactive
    // ULK = FALSE; // Unlock function inactive
    // if(L==TRUE){R=FALSE;} // L and R inputs cannot both be TRUE on the same cycle
	
    f1();

    // if F is disabled, left, right, and warning functions are deactivated
    if (!F)
      assert(outL==FALSE && outR==FALSE);
...
int main() {
  ...
  inputs[4][5] = 1; // activation du warning
  //  inputs[14][5] = 1; // desactivation du warning

  //  inputs[0][3] = 1; // fermeture des portes (déjà fermées)
  //  inputs[0][4] = 1; // ouverture des portes
